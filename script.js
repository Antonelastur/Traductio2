...
document.addEventListener('DOMContentLoaded', () => {
    ...
    pdfUpload.addEventListener('change', async (e) => {
        try {
            sourceText.value = "[Event Change] S-a selectat un fișier...";
    ...
    translateBtn.addEventListener('click', async () => {
        ...
        const tmMatches = window.TMDB.findMatches(textToTranslate, currentDomain, sl, tl);

        // Afișăm matches în UI (Dacă există)
        tmMatchList.innerHTML = '';
        ...
        const tmPromptContext = window.TMDB.formatMatchesForPrompt(tmMatches);
        ...
        try {
            ...
            window.TMDB.addTranslation(sl, tl, currentDomain, textToTranslate, translatedResult);

        } catch (error) {
            console.error(error);
            if (error.message === "API_KEY_MISSING") {
                targetText.innerHTML = `<span style="color: #ef4444;">Eroare: Nu aveți setat un API Key pentru Gemini.</span>`;
                settingsModal.classList.remove('hidden'); // Opertim modulul de setări
            } else {
